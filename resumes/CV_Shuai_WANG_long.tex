...
%\cvitem{Machine Learning}{regression, classification, clustering} 
\cvitem{AutoML}{Datarobot, H2O; Pycaret, GlutonTS}
\cvitem{Optimization Modeling}{AIMMS, Pyomo, Minizinc, JuMP, OPL; Cplex, Gurobi, CBC, HiGHS }
\cvitem{GenAI}{Snowflake Cortex, GPRO, PRO, LangChain }
\cvitem{Heuristic}{Simulated Annealing, Genetic Algorithm, Tabu Search, Golden Section Search, etc.}
\cvitem{Time Series}{Anomaly detection, Forecasting, Correlation}
...
\section{Experience}
\cventry{2023.6--Current} {Senior Data Scientist(Manager Level), Full time}{\textsc{Pacific Life Insurance}}{Newport Beach, CA}{}{}

Creating roadmap and portfolio for data science projects on operation, Generative AI, customer service areas. Supporting sales area. 

Leading data science projects in the tech areas of math optimization, forecasting, and function areas in operations, customer service, call center, and marketing. Mentoring junior data scientist.

\begin{itemize}
\item Developed various time series forecasting using AutoML, for processing task volume for annuity and life products.

\item Created a MILP based skill-based routing task assignment system, by using HiGHS solver via Pyomo, to optimize tasks dispatching by considering specialist competence, task priority, time constraints, etc. Productivity improved 10 to 20 percent across business functions. 
...
\item Created a GenAI tool for customer service and operation by using customer voice data. Implemented using Snowflake's cortex LLM suit. 
\item Implemented virtual queue, call back policy to call center.
\item Re-created mortality table(life table) using math optimization. 
...
\cventry{2021.4--2023.5} {Principal Data Scientist(Tech Lead), Full time}{\textsc{Thomasnet}}{New York, NY}{}{
...
\item \textbf{Internal facing:}
    \begin{enumerate}
    \item Applied AutoML to models like customer churn, bot detection, and sales forecasting.
    \item Recommending products portfolio/ad placement to customers(suppliers) to maximize ROI like views  and inquiries,  using integer programming optimization.
...
\bigskip
\cventry{2017.9--2021.4}{Lead Operations Research Scientist, Contract through Global Associates}
...
The project is to transform the in-store inventory into science-based management system, to increase productivity:
...
jointly developed with \textsc{Kearney} using AIMMS to build promotion planning and
assortment selection to maximize the adjusted gross revenue.
The system is implemented in 2014 and saved about \$400 millions. 
%\newline{}
\item \textbf{Forecast and Optimization for the Little Clinics and Nurse Scheduling}: The problem is
to accurately forecast the number of patients by type to each clinic, then to create a scheduling system.
...
out-of-stock.
 ...
 \item \textbf{Periodic Vehicle Routing}: Each store has demand that
...
\item \textbf{Dissertation Committee:}
    \begin{enumerate}
\item Xinhui Zhang(Supervisor): Senior Director of Data Science at Coupang. Three times INFORMS Franz Edelman Award finalist.
